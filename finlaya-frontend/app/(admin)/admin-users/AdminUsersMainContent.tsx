...
export default function AdminUsersMainContent() {
  ...
  const openViewModal = async (u: User) => {
    ...
    const [{ count: expenses }, { count: income }, { count: goals }, { count: bills }] = await Promise.all([
  ...
  const handleDelete = async () => {
    ...
    setDeleteError('');
    try {
      const res = await fetch(`${BACKEND_URL}/api/admin/users/${deleteTarget.user_id}`, { method: 'DELETE' });
      if (!res.ok) {
    ...
    <div className="min-h-screen bg-[#0d1117]">
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="relative mb-5">
          ...
          <input
            ...
            className="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl pl-10 pr-4 py-2.5 text-sm text-white/70 placeholder-white/20 outline-none focus:border-orange-500/40 focus:bg-white/[0.06] transition-all"
          />
        </div>
        ...
        <div className="border border-white/[0.08] rounded-2xl overflow-hidden">
          ...
          {/* Column headers */}
          <div className="px-6 py-3.5 grid grid-cols-12 bg-white/[0.03] border-b border-white/[0.08]">
            ...
            <span className="col-span-2 text-[11px] text-white/30 uppercase tracking-widest font-semibold hidden lg:block">Joined</span>
            <span className="col-span-2 text-[11px] text-white/30 uppercase tracking-widest font-semibold text-right">Actions</span>
          </div>
          ...
          {loading ? (
            ...
            <div className="py-16 text-center">
              <Users size={20} className="text-white/15 mx-auto mb-3" />
            ...
            <div>
              {filtered.map((u, i) => (
                <div
                  key={u.user_id}
                  className={`px-6 py-4 grid grid-cols-12 items-center hover:bg-white/[0.03] transition-colors ${
                ...
                >
                  ...
                  <div className="col-span-4 flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                      ...
                      {isSelf(u) && (
                        <span className="text-[9px] font-semibold text-orange-400 bg-orange-500/10 border border-orange-500/20 px-1.5 py-0.5 rounded-full flex-shrink-0">
                          YOU
                        </span>
                      )}
                    </div>
                  ...
                  <div className="col-span-2 hidden lg:block">
                    <span className="text-white/30 text-xs">
                      {new Date(u.registration_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                    </span>
                  </div>
                  ...
                  <div className="col-span-2 flex items-center justify-end gap-1">
                    ...
                    <button
                      ...
                      className={`p-2 rounded-lg transition-colors ${
                        isSelf(u) ? 'text-white/15 cursor-not-allowed' : 'text-white/30 hover:text-red-400 hover:bg-red-500/10'
                      }`}
                    >
                      <Trash2 size={13} />
                    </button>
      ...
      {viewUser && (
        <div className="fixed inset-0 z-40 flex items-center justify-center px-4">
          ...
          <div className="relative bg-[#13171f] border border-white/[0.1] rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">

            <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.08]">
              <span className="text-white/80 text-sm font-semibold">User Profile</span>
              <button onClick={() => setViewUser(null)} className="p-1.5 rounded-lg text-white/30 hover:text-white hover:bg-white/[0.06] transition-colors">
                <X size={15} />
              </button>
            </div>
            ...
            <div className="px-6 py-5 border-b border-white/[0.08]">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-orange-500/15 border-2 border-orange-500/25 flex items-center justify-center text-orange-400 text-base font-bold">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {isSelf(viewUser) && (
                      <span className="text-[9px] font-semibold text-orange-400 bg-orange-500/10 border border-orange-500/20 px-1.5 py-0.5 rounded-full">
                        YOU
                      </span>
                    )}
                  </div>
            ...
            <div className="px-6 py-5 grid grid-cols-2 gap-4 border-b border-white/[0.08]">
              {[
                { icon: Mail, label: 'Email', value: viewUser.email },
                { icon: Calendar, label: 'Joined', value: new Date(viewUser.registration_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) },
                ...
                <div key={label} className="flex items-start gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-white/60 text-xs break-all">{value}</p>
                  </div>
                </div>
              ))}
              <div className="flex items-start gap-2.5 col-span-2">
                <DollarSign size={13} className="text-white/25 mt-0.5 flex-shrink-0" />
                <div>
                  <p className="text-[10px] text-white/30 uppercase tracking-wider mb-0.5">Monthly Salary</p>
              ...
              {statsLoading ? (
                ...
                <div className="grid grid-cols-4 gap-2">
                  {[
                    ...
                    { label: 'Goals', value: userStats.goals, icon: Target, color: 'text-amber-400' },
            ...
            <div className="px-6 py-4 flex items-center justify-between">
              {isSelf(viewUser) ? (
                <span className="text-white/20 text-xs flex items-center gap-1.5">
                  <Trash2 size={11} /> Cannot delete your own account
                </span>
              ) : (
                <button
              ...
              <button onClick={() => setViewUser(null)} className="px-4 py-2 rounded-lg bg-white/[0.06] hover:bg-white/[0.1] text-white/50 hover:text-white/80 text-sm transition-colors">
                Close
              </button>
            </div>
          </div>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
          ...
          <div className="relative bg-[#13171f] border border-white/[0.1] rounded-2xl p-6 w-full max-w-sm shadow-2xl">
            <div className="w-10 h-10 rounded-xl bg-red-500/10 border border-red-500/20 flex items-center justify-center mb-4">
              <Trash2 size={16} className="text-red-400" />
            </div>
            <h3 className="text-white font-semibold text-base mb-1">Delete user?</h3>
            <p className="text-white/40 text-sm mb-1">
              <span className="text-white/70 font-medium">{deleteTarget.username}</span> and all their data will be permanently removed.
            </p>
            ...
            <div className="flex gap-2.5">
              <button
                ...
                disabled={deleting}
                className="flex-1 py-2.5 rounded-xl border border-white/[0.1] text-white/40 hover:text-white/70 hover:bg-white/[0.06] text-sm transition-colors disabled:opacity-40"
              >
                Cancel
              </button>
              <button
