...
const AvatarCircle = ({
  avatarUrl, initials, size = 'sm',
}: {
  avatarUrl: string | null; initials: string; size?: 'sm' | 'lg';
}) => {
  ...
  return (
    <div className={`${dimension} rounded-full bg-gradient-to-br from-amber-400 to-orange-500 flex items-center justify-center text-white font-semibold shadow-sm`}>
      {initials}
    </div>
  );
};
  ...
  useEffect(() => {
    const fetchAvatar = async () => {
      ...
      const { data } = await supabase
        .from('users').select('avatar_url').eq('user_id', user.id).maybeSingle();
