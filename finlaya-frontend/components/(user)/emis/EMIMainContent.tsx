...
export default function EMIMainContent() {
  ...
  const [rebalanceDialog, setRebalanceDialog] = useState<{
    isOpen: boolean; overflowAmount: number; newBudgetable: number;
  ...
  // ── Check category overflow after saving a loan ────────────────────────────
  ...
  const checkCategoryOverflow = async () => {
    ...
    if (totalCatBudget > 0 && totalCatBudget > newBudgetable) {
      setRebalanceDialog({
        isOpen: true,
        overflowAmount: totalCatBudget - newBudgetable,
        newBudgetable,
      });
    }
  };

  // ── Auto-scale categories to fit new budgetable salary ────────────────────
  ...
  const handleAutoScale = async () => {
    ...
    // Largest-remainder method so budgets sum exactly to newBudgetable
    ...
    const sortedByFrac = scaled
      .map((c, i) => ({ i, frac: c.exact - floored[i].budget }))
  ...
  const doSave = async (
    ...
    if (!user?.id) return;
    if (editingEMISnap) {
      await supabase.from('emi_payments')
        .update({ ...data, updated_at: new Date().toISOString() })
        .eq('emi_id', editingEMISnap.emi_id).eq('user_id', user.id);
    } else {
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        <EMITable
          ...
          onTogglePaid={handleTogglePaid} onAdd={handleAdd}
        />
      </div>
      ...
      {/* Mark paid despite low balance */}
      <ConfirmDialog
        isOpen={toggleConfirm.isOpen}
        title="Low balance warning"
        message={toggleConfirm.emi
      ...
      {/* Categories overflow after loan was added */}
      <RebalanceDialog
